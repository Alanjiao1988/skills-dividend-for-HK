...
For JSON, `cash_election_available` describes the particular scheme, not every distribution by the issuer. A mandatory stock scheme with `"No"` can coexist with a separate cash dividend. Use `investor_cash_yield_assumption: "cash_components_only"` and the dated event reconciliation in `business-fundamentals.md` for mixed schedules; count only supported cash receipts and track subsequent issuer and investor share entitlements. For ordinary cash dividends without a scrip/DRIP scheme, use `available: "No"`, `cash_election_available: "Not Applicable"` and `investor_cash_yield_assumption: "not_applicable"`. An available cash election uses `"Yes"` and `"all_cash_election"`; the default election may still be shares. Unknown cash access remains unassessed. A wholly stock-only schedule has no cash-income valuation; a mixed schedule retains its legitimate cash component.
