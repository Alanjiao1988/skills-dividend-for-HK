...
- `ruleset` records the rules the report was produced under. New analyses from this skill use `ruleset: "2.3"` with `summary_provenance: "repaired_with_evidence"`. Before publishing, verify the archive schema and validator accept that ruleset; if they still allow only `pre-2.2` and `2.2`, stop publication and identify the required archive migration. Never relabel a 2.3 report as 2.2 to bypass validation. Keep transcription evidence mandatory for new rulesets. Existing `2.2` and `pre-2.2` entries retain their original labels; `pre-2.2` predates the retired sector yield presets and the five-year outlook requirement, so its required yields, entry bands, portfolio roles and veto labels are historical records, not current results. Do not reuse them as this skill's output or treat their no-entry frequency as threshold calibration. The archive's `MIGRATION.md` lists the historical differences.
